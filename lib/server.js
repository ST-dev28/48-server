...
const adminServicesPageHandler = require('./handlers/admin-service');
...
server.httpServer = http.createServer((req, res) => {
    ...
    req.on('end', async (data) => {
        ...
        if (isTextFile || isBinaryFile) {   // tekstiniai failai ir nuotraukos bei sriftai
            let fileContent = '';
            if (isTextFile) {
            ...
            if (fileContent === '') {
                ...
                res.writeHead(200, {
                    'Content-Type': MIMES[fileExtension],
                    'Cache-Control': 'max-age=3000000',   // kiek sekundziu ilgiausiai galima atminty laikyti atisiusta resursa (siuo atveju informacija butu saugoma apie menesi)
                })
            }
